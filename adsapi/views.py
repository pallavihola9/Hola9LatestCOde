...
class NotificationAPIView(APIView):
    def get(self, request, PlanCategory, *args, **kwargs):
        # Assuming you want to retrieve notifications for a specific action type
        user = request.user  # Assuming authentication is used

        Notification.objects.filter(
            user=user,
            
            is_read=False  # You might want to filter based on read status or other criteria
        )

        # Serialize notifications and return the response
        # ...

        return Response({"message": "Notifications retrieved successfully"}, status=status.HTTP_200_OK)

    def post(self, request, PlanCategory, *args, **kwargs):
        user = request.user  # Assuming authentication is used

        if PlanCategory == 'purchase_plan':
            ...
            Notification.objects.create(
                user=user,
                # action_type=PlanCategory,
                target_object_id=PlanCategory,  # Replace with the actual ID of the purchased plan
                content="You purchased a plan."
            )

            return Response({"message": "Plan purchased successfully"}, status=status.HTTP_201_CREATED)

        elif PlanCategory == 'post_ad':
            # Your ad posting logic here
            # ...
            ...
            Notification.objects.create(
                user=user,
                # action_type=PlanCategory,
                target_object_id=ad_id,  # Replace with the actual ID of the posted ad
                content="You posted an ad."
            )

            return Response({"message": "Ad posted successfully"}, status=status.HTTP_201_CREATED)

        elif PlanCategory == 'add_comment':
            # Your comment logic here
            # ...

            # Assuming the comment is related to an ad
            ad_id = request.data.get('ad_id')  # Adjust based on how you send the ad_id in the request

            # Create a comment
            AdsComment.objects.create(
                ads_id=ad_id,
                email=request.data.get('email'),
                datetimeValue=request.data.get('datetimeValue'),
                message=request.data.get('message')
            )
            ...
            Notification.objects.create(
                ...
                content="You added a comment on an ad."
            )
